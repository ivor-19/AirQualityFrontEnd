...
export const AuthProvider = ({ children }) => {
  ...
  useEffect(() => {
    const loadData = async () => {
      try {
        // Fetch the stored token and other data
        const storedToken = await SecureStore.getItemAsync('userToken');
        if (!storedToken) {
          console.log("No user token found, redirecting to login screen");
        ...
        if (storedToken && storedUserId && storedUsername && storedEmail && storedAssetModel && storedFirstAccess) {
          ...
          console.log("Incomplete user data, redirecting to login screen");
          router.replace('landingPage');
        }
      } catch (error) {
        console.error('Error loading data from SecureStore:', error);
        router.replace('landingPage'); // Redirect to login if an error occurs
      }
    };
